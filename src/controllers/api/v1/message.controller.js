...
exports.index = catchAsync(async (req, res) => {
  try {
    ...
    await Message.paginate({ conversation_id }, { offset, limit, sort: { createdAt: 'desc' } }).then((result) => {
...
exports.sendToUser = catchAsync(async (req, res) => {
  ...
  // Create conversation
  const conversation = await conversationService.createConversation(null, user_sender ? user_sender._id : sender._id, [
    user_receiver ? user_receiver._id : receiver._id,
    user_sender ? user_sender._id : sender._id,
  ]);
  // Create message
...
exports.update = catchAsync(async (req, res) => {
  try {
    ...
    await Message.findByIdAndUpdate(message_id, update, { useFindAndModify: false })
      .then((data) => {
        if (!data) throw new ApiError(httpStatus.NOT_FOUND, 'Message not found');
        else {
...
exports.notification = catchAsync(async (req, res) => {
  ...
  await admin
    .messaging()
    .sendToDevice(participant.device_tokens, message, options)
    .then((resp) => {
      res.send(resp);
    })
    .catch((error) => {
